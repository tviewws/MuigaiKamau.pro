...
const articles = [
  {
    number: "01", category: "Operational Transformation", date: "April 2026", readTime: "8 min read",
    image: Perspective1, imageAlt: "Operations team reviewing performance metrics on screens",
    title: "Why Most Operational Transformation Programs Fail in Africa — And What Actually Works",
    intro: "Across Africa, organizations are investing heavily in operational transformation. ERP systems are implemented, consultants are hired, strategy decks are produced, and ambitious targets are announced. Yet, despite these efforts, many transformation programs fail to deliver lasting value. Costs remain high, service levels stagnate, working capital remains trapped, and teams often revert to old habits once external support exits.",
    sections: [
      {
        heading: "The Root Causes of Failure", type: "numbered",
        items: [
      ...
      {
        heading: "What Actually Works in African Contexts", type: "bullets",
        items: [
  ...
  {
    number: "02", category: "M&A & Due Diligence", date: "March 2026", readTime: "9 min read",
    image: Perspective2, imageAlt: "Business executives shaking hands after a deal",
    ...
    sections: [
      ...
      {
        heading: "Common Operational Blind Spots", type: "bullets",
        items: [
  ...
  {
    number: "03", category: "Strategic Execution", date: "February 2026", readTime: "7 min read",
    image: Perspective3, imageAlt: "Leadership team in a strategic planning session",
    ...
    sections: [
      ...
      {
        heading: "Why the Gap Persists", type: "bullets",
        items: [
          ...
          <div key={i} className="flex gap-4">
            <div className="flex-shrink-0 mt-1">
              {section.type === "numbered" ? (
                <span className="w-6 h-6 rounded-full bg-[#F59E0B] text-[#0F172A] text-[11px] font-bold flex items-center justify-center">{i + 1}</span>
              ) : (
...
function ArticleCard({ article, align }: { article: any; align: "left" | "right" }) {
  ...
  return (
    <div className={`flex ${align === "right" ? "justify-end" : "justify-start"}`}>
      <article className="w-full md:w-[78%] bg-white rounded-xl border border-[#0F172A]/10 overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300"
        style={{ borderLeft: align === "left" ? "4px solid #F59E0B" : undefined, borderRight: align === "right" ? "4px solid #F59E0B" : undefined }}>

        <div className="w-full h-48 overflow-hidden relative">
          <img src={article.image} alt={article.imageAlt} className="w-full h-full object-cover" loading="lazy" />
          <div className="absolute inset-0 bg-gradient-to-t from-[#0F172A]/60 to-transparent" />
          <span className="absolute bottom-3 left-4 bg-[#F59E0B] text-[#0F172A] px-3 py-1 rounded text-[11px] font-bold uppercase tracking-wider">{article.category}</span>
        </div>

        <button onClick={() => setExpanded((e) => !e)} className="w-full text-left p-8 md:p-10 flex items-start justify-between gap-6 group">
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-3 mb-4">
              <span className="text-[#0F172A]/40 text-[12px] font-bold">#{article.number}</span>
              <span className="flex items-center gap-1 text-[#1F2937]/50 text-[12px]"><Calendar className="w-3 h-3" /> {article.date}</span>
              <span className="flex items-center gap-1 text-[#1F2937]/50 text-[12px]"><Clock className="w-3 h-3" /> {article.readTime}</span>
            </div>
            <h2 className="text-[#0F172A] font-bold text-[22px] sm:text-[26px] leading-snug group-hover:text-[#F59E0B] transition-colors">{article.title}</h2>
            <p className="text-[#1F2937] text-[15px] leading-relaxed mt-4 max-w-3xl">{article.intro}</p>
          </div>
          <div className="flex-shrink-0 mt-1">
            <div className="w-9 h-9 rounded-full border-2 border-[#0F172A]/20 flex items-center justify-center group-hover:border-[#F59E0B]"
        ...
        <div style={{ display: "grid", gridTemplateRows: expanded ? "1fr" : "0fr", transition: "0.4s" }}>
          <div style={{ overflow: "hidden" }}>
            <div className="px-8 md:px-10 pb-10 border-t border-[#0F172A]/8">
              <div className="pt-8">
                {article.sections.map((section: any, i: number) => <ArticleSection key={i} section={section} />)}
                <div className="mt-8 p-6 bg-[#0F172A] rounded-xl">
                  <p className="text-[#F59E0B] font-semibold text-[13px] uppercase tracking-widest mb-2">The Bottom Line</p>
...
export function Perspectives() {
  return (
    <div className="bg-white">
      <SEO
        title="Our Perspectives | MK Consulting & Advisory"
        description="Thought leadership on operational transformation, M&A due diligence, and strategic execution in African markets. Insights from MK Consulting & Advisory."
        canonical="https://www.muigaikamau.pro/perspectives"
      />
